...
class SQLToolCapability(ToolCapability):
    def __init__(self, repository: Any, default_dialect: str = "oracle") -> None:
    ...
    def description(self) -> str:
        return "Validate and execute SQL queries. Defaults to Oracle dialect."
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                id=call_id, output="", error=f"SQL validation error ({dialect}): {exc}"
            )
        ...
        try:
            result = await self._repository.execute(transpiled)
